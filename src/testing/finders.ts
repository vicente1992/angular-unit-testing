...
export function geText<T>(fixture: ComponentFixture<T>, testId: string) {
...
export function query<T>(fixture: ComponentFixture<T>, selector: string) {
  ...
  if (!debugElement) {
    throw new Error(`query: elemento con el selector ${selector} no encontrado`);
  }
  return debugElement;
}
...
export function queryAllByDirective<T, D>(fixture: ComponentFixture<T>, selector: Type<D>) {
